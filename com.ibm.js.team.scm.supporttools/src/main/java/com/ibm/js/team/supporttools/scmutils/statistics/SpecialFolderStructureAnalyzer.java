...
public class SpecialFolderStructureAnalyzer {
	...
	private boolean fLatestOnly=false;
	...
	public void analyze(File sandboxFolder) {
		...
		if(sandboxFolder.isDirectory()){
			analyzeSandboxFolder(sandboxFolder, "", comp, 0);
		}
	}
	 ...
	 * @param depth
	 */
	private void analyzeSandboxFolder(File sandboxFolder, String path, ComponentStat compStat, int depth) {
		...
		for (File file : contents) {
			if (file.isDirectory()) {
				if (!isIgnoredDirectory(file)) {
					...
					analyzeProjectFolder(file, file.getAbsolutePath(), new ExportInformation(file.getName()), compStat, depth + 1);
				} else {
	 ...
	 * @param depth
	 */
	private void analyzeProjectFolder(File project, String absolutePath, ExportInformation exportInformation, ComponentStat compStat, int depth) {
		...
		for (File file : contents) {
			if (file.isDirectory()) {
				if (!isIgnoredDirectory(file)) {
					folders++;
					compStat.addFolderStat(file, depth);
					if("Export".equalsIgnoreCase(file.getName())){
						analyzeExportFolder(file, file.getAbsolutePath(), exportInformation, compStat, depth + 1);						
					} else {
						// Uninteresting
					}
				} else {
	 ...
	 * @param depth
	 */
	private void analyzeExportFolder(File exportFolder, String absolutePath, ExportInformation exportInformation, ComponentStat compStat, int depth) {
		...
		for (File file : contents) {
			if (file.isDirectory()) {
				if (!isIgnoredDirectory(file)) {
					folders++;
					compStat.addFolderStat(file, depth);
					exportInformation.setSupplier(file.getName());
					analyzeSupplierFolder(file, file.getAbsolutePath(), exportInformation, compStat, depth + 1);						
				} else {
					logger.info("\nIgnoring folder '{}'", file.getAbsolutePath());
				}
			} else {
